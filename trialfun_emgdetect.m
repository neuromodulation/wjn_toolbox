...
iout = [];
for a = 1:length(emgon)
    try
        nemgoff(a) = emgoff(find(emgoff>emgon(a),1,'first'));
    catch
        iout=[iout a];
    end
end
...
trl(:,1) = emgon(:) + hdr.Fs*0.5-hdr.Fs;  % as a consequence of the convolution with a one-second boxcar
trl(:,2) = emgon(:) + hdr.Fs*.5 + hdr.Fs;  % as a consequence of the convolution with a one-second boxcar
trl(:,3) = -hdr.Fs;