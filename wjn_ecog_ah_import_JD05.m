...
for a = 1:Dmove.nchannels-2
    ...
    drawnow
    myprint(['TF_move_' id '_' Dmove.chanlabels{a}])
end
...
smspac = squeeze(nanmean(Dp(ci(sSTN,Dp.chanlabels),:,:,1),1).*(Dp.p(1,:,:,1)<=0.05));
smlpac = squeeze(nanmean(Dp(ci(sSTN,Dp.chanlabels),:,:,2),1).*(Dp.p(1,:,:,2)<=0.05));

%
ich = ci('Strip3',Dp.chanlabels);

cmspac = squeeze(nanmean(Dp(ich,:,:,1),1).*(Dp.p(ich,:,:,1)<=0.05));
cmlpac = squeeze(nanmean(Dp(ich,:,:,2),1).*(Dp.p(ich,:,:,2)<=0.05));
