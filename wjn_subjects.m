function [files, sequence, root, details] =wjn_subjects(initials, on)
...
details.berlin = isequal(initials(1:4), 'PLFP') || isequal(initials(1:4), 'DYST') || isequal(initials(1:3), 'WJN');
...
switch initials
	...
	case 'PLFP31'
		if on
			files = {
				'PLFP31/230413on/Raw/plfp31.0100.con',...
				'PLFP31/230413on/Raw/plfp31.0200.con'
			};
			root = 'PLFP31/230413on/';
			sequence = {'R','EMOT'};
			details.lfpthresh = 0.3;
			details.berlin = 1;
    ...
    case 'PLFP32'
		if on
			files = {
				'PLFP32/050713on/Raw/plfp32.0100.con',...
				'PLFP32/050713on/Raw/plfp32.0200.con',...
				'PLFP32/050713on/Raw/plfp32.0300.con',...
				'PLFP32/050713on/Raw/plfp32.0400.con'
			};
	...
	case 'PLFP33'
		if on
			files = {
				'PLFP33/230813on/Raw/plfp33.0100.con',...
				'PLFP33/230813on/Raw/plfp33.0200.con'
			};
			root = 'PLFP33/230813on/';
			sequence = {'R','EMOT'};
			details.lfpthresh = 0.3;
			details.berlin = 1;
	...
	case 'PLFP34'
		if on
			files = {
				'PLFP34/130913on/Raw/plfp34.0100.con',...
				'PLFP34/130913on/Raw/plfp34.0200.con'
			};
			root = 'PLFP34/130913on/';
			sequence = {'R','EMOT'};
			details.lfpthresh = 0.3;
			details.berlin = 1;
    ...
    case 'PLFP35' % Boston Sci
		if on
			files = {
				'PLFP35/291013on/Raw/plfp35.0100.con',...
				'PLFP35/291013on/Raw/plfp35.0200.con'
			};
			root = 'PLFP35/291013on/';
			sequence = {'R','STOP'};
			details.lfpthresh = 0.3;
			details.berlin = 1;
	...
	case 'PLFP36' % Boston Sci
		if ~on
			files = {
				'PLFP36/081113on/Raw/plfp36.0100.con',...
				'PLFP36/081113on/Raw/plfp36.0200.con',...
				'PLFP36/081113on/Raw/plfp36.0300.con',...
				'PLFP36/081113on/Raw/plfp36.0400.con'
			};
			root = 'PLFP36/081113on/';
	...
	case 'PLFP37'
		if ~on
			files = {
				'PLFP37/251113on/Raw/plfp37.0100.con',...
				'PLFP37/251113on/Raw/plfp37.0200.con',...
				'PLFP37/251113on/Raw/plfp37.0300.con'
			};
			root = 'PLFP37/251113on/';
	...
	case 'PLFP38' % different motor/trigger channels
        if on
			files = {
				'PLFP38/070314on/Raw/plfp38.0100.con',...
				'PLFP38/070314on/Raw/plfp38.0200.con',...
				'PLFP38/070314on/Raw/plfp38.0300.con'
			};
			root = 'PLFP38/070314on/';
	...
	case 'PLFP39' % different motor/trigger channels
		if on
			files = {
				'PLFP39/240314on/Raw/plfp39.0100.con',...
				'PLFP39/240314on/Raw/plfp39.0200.con'
			};
			root = 'PLFP39/240314on/';
			sequence = {'R','MOTOR'};
			details.lfpthresh = 0.3;
			details.berlin = 1;
            ...
            for f=1:length(files)
                ...
                if ~exist([pathstr filesep  name '.gain.txt'], 'file');
                    try
                        ...
                        copyfile('D:\MEG\plfp_spm\SPM\gain.txt',[pathstr filesep name '.gain.txt'])
                        warning('Could not get gain information taking old file.')
                    end
                end
            end
    ...
    case 'PLFP41'
        if on
           ...
           files = {
            ...
            sequence = {'R','EMOT','EMOT'};
            details.berlin = 1;
            details.electrode = 'boston_segmented';
...
%             details.right_trigger = 'EEG160';
    ...
    case 'PLFP43'
		if on
            ...
            [montage, label]	 = berlin_make_montage_bostsci_seg_ring(dbsroot, initials, '.0200');
            details.chanset = label;
          files = {
    ...
    case 'PLFP45'
		if on
         ...
         case 'WJN01'
           files = {
                'WJN01/wjn_rest.0100.con',...
               };
            root = 'WJN01/';

            sequence = {'R'};
            details.berlin = 1;
            details.markers(1).files = {'wjn_rest.before1-coregis.txt','wjn_rest.after1-coregis.txt'};
            details.flatthresh = 25;  
            for f=1:length(files)
                [pathstr, name, ext] = fileparts([dbsroot filesep files{f}]);
                if ~exist([pathstr filesep  name '.gain.txt'], 'file')
                    copyfile('D:\MEG\plfp1_spm\SPM\gain.txt',[pathstr filesep name '.gain.txt'] )
                end
            end
            details.orestfile = fullfile(outroot,root,'SPMrest','tsssDYST04_on.mat');
            details.left_trigger = 'EEG159';
...
if ~isfield(details,'electrode')
    chan = {'LFP_R01', 'LFP_R12', 'LFP_R23', 'LFP_L01', 'LFP_L12', 'LFP_L23'};
elseif strcmp(details.electrode,'boston_segmented')
    chan = {'LFP_R18','LFP_R23','LFP_R24','LFP_R25', 'LFP_R34','LFP_R36', 'LFP_R47','LFP_R56', 'LFP_R57', 'LFP_R67', ...
        'LFP_L18', 'LFP_L23', 'LFP_L24', 'LFP_L25', 'LFP_L34', 'LFP_L36', 'LFP_L47', 'LFP_L56', 'LFP_L57', 'LFP_L67'};
elseif strcmp(details.electrode,'boston_ring')
    chan = {'LFP_R01', 'LFP_R12', 'LFP_R23','LFP_R34','LFP_R45','LFP_R56','LFP_R78',...
        'LFP_L01', 'LFP_L12', 'LFP_L23','LFP_L34', 'LFP_L45', 'LFP_L56', 'LFP_L67','LFP_L78'};
end
