function imagefiles = source_coherence_mesh(folder,file,refchans,freqranges,condition,nshuffle,normalisation,flip)
% 
% source_coherence(folder,file,refchans,freqranges,condition,nshuffle,normalisation,flip)
D=spm_eeg_load(file);
if ~iscell(refchans);
...
for a=1:numel(refchans);
    for b = 1:size(fw,1);
        cd(folder);
        dirname = ['mesh_' refchans{a} '_' num2str(fw(b,1)) '_' num2str(fw(b,2))];
        mkdir(dirname);
   
        matlabbatch{1}.spm.tools.beamforming.data.dir = {[folder '\' dirname]};
        ...
        if ~flip
            matlabbatch{2}.spm.tools.beamforming.sources.plugin.mesh.flip = true;
        else
            matlabbatch{2}.spm.tools.beamforming.sources.plugin.mesh.flip = true;
        end
     ...
     for shuffle = 0:nshuffle;
            if shuffle
                Q = fullfile(folder,dirname,['m_' condition '_dics_refcoh_shuffled_' fname '_' num2str(shuffle) '.gii']);
                if ~exist(Q,'file')
                ...
                if normalisation
                    matlabbatch{6}.spm.tools.beamforming.write.plugin.gifti.normalise = 'separate';
                else
                    matlabbatch{6}.spm.tools.beamforming.write.plugin.gifti.normalise = 'no';
                end
         
                spm_jobman('run',matlabbatch);
                end
                %% rename shuffled file
%                 movefile(fullfile(folder,dirname,['dics_refcoh_shuffled_' fname '.gii']),fullfile(folder,dirname,[condition '_dics_refcoh_shuffled_' fname '_' num2str(shuffle) '.gii']))
               
%                 %% mask Source coherence from EEG Template
%                 Vi = {fullfile(folder,dirname,[condition '_dics_refcoh_shuffled_' fname '_' num2str(shuffle) '.gii']);...
%                     fullfile(spm('dir'),'EEGtemplates','iskull.gii')};
%                 Vo = fullfile(folder,dirname,['m_' condition '_dics_refcoh_shuffled_' fname '_' num2str(shuffle) '.gii']);
%                 fcalc = 'i1.*(i2>1)';
%                 spm_imcalc(Vi,Vo,fcalc)
%                 end
%                 %% smooth
%                 P = fullfile(folder,dirname,['m_' condition '_dics_refcoh_shuffled_' fname '_' num2str(shuffle) '.gii']);
%                 Q = fullfile(folder,dirname,['sm_' condition '_dics_refcoh_shuffled_' fname '_' num2str(shuffle) '.gii']);
%                 spm_smooth(P,Q,[16 16 16]);
%                
%                 sfname{shuffle} = Q;
            else
                Q = fullfile(folder,dirname,['m_' condition '_dics_refcoh_' fname '.gii']);
                if ~exist(Q,'file')
                matlabbatch{5}.spm.tools.beamforming.output.BF(1) = cfg_dep('Inverse solution: BF.mat file', substruct('.','val', '{}',{4}, '.','val', '{}',{1}, '.','val', '{}',{1}, '.','val', '{}',{1}), substruct('.','BF'));
                matlabbatch{5}.spm.tools.beamforming.output.plugin.image_dics.reference.refchan.name = refchans{a};
               ...
               %% mask Source coherence from EEG Template
%                  Vi =[];Vo=[];
%                  oname = fullfile(folder,dirname,['dics_refcoh_' fname '.gii']);
%                  nname = fullfile(folder,dirname,[condition '_dics_refcoh_' fname '.gii']);
%                  movefile(oname,nname)
%                 Vi = {nname;...
%                     fullfile(spm('dir'),'EEGtemplates','iskull.gii')};
%                 Vo = fullfile(folder,dirname,['m_' condition '_dics_refcoh_' fname '.gii']);
%                 f = 'i1.*(i2>1)';
%                 spm_imcalc(Vi,Vo,f)
                end
                %% Smooth
%                 P = fullfile(folder,dirname,['m_' condition '_dics_refcoh_' fname '.gii']);
%                 Q = fullfile(folder,dirname,['sm_' condition '_dics_refcoh_' fname '.gii']);
%                 spm_smooth(P,Q,[16 16 16]);
                
%                 ofname = fullfile(folder,dirname,[fname '.mat.surf.gii']);
            end
        end
%         clear matlabbatch
%         
...
%             load(fullfile('C:\matlab\meg_toolbox', 'cohimage_ttest_job.mat'));
%              spm_jobman('initcfg')
...
%             matlabbatch{1}.spm.stats.factorial_design.masking.em = {'C:\SPM12\EEGtemplates\iskull.gii'};
