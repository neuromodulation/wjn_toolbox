function source_extraction_dir_nomeg(folder,file,condition,emgchans,lfpchans,position,freqrange,target_name,source_name,radius,side)
...
if exist('side','var') && strcmp('side','left');
    pos(1) = pos(1)*-1;
end
...
if ~iscell(source_name)
    source_name = cellstr(source_name);
end
...
matlabbatch{2}.spm.tools.beamforming.sources.plugin.voi.vois{a}.voidef.label = source_name{a} ;
matlabbatch{2}.spm.tools.beamforming.sources.plugin.voi.vois{a}.voidef.pos = position(a,:);
