function [b,e] = mybarh(y,cmap,x,bwidth,ec)
...
%     keyboard
       b(a) = barh(x,ny,'FaceColor',cmap(a,:),'barwidth',bwidth,'Edgecolor',ec);
...
for a = 1:length(my)
    ...
    %,'LineStyle','none','color',bc,'Marker','none','Capsize',0
    if my(a)<0
%         e=herrorbar(x(a),my(a),sy(a),zeros(size(sy(a))));
    elseif my(a)>0
%         e=herrorbar(x(a),my(a),zeros(size(sy(a))),sy(a));
        %% ,'LineStyle','none','color',bc,'Marker','none','Capsize',0
%     else
