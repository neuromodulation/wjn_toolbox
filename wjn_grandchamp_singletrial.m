function nD=wjn_grandchamp_singletrial(filename,baseline,method,condition)
% nD=wjn_grandchamp_singletrial(filename,baseline,method,condition)
    ...
    for a = 1:nc
        for b = 1:nf
            for c  = 1:nt
                ...
                if strcmp(method,'additive')
                    d(a,b,:,c) = (D(a,b,:,c)-squeeze(nanmean(D(a,b,ib,cc),3)))./nanstd(squeeze(D(a,b,ib,cc)));
                elseif strcmp(method,'gain')
                    d(a,b,:,c) = 100*((D(a,b,:,c)-squeeze(nanmean(D(a,b,ib,cc),3)))./squeeze(nanmean(D(a,b,ib,cc),3)));
                end
            end
        end
    end
