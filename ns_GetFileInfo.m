...
for i = 1:length(hFile.FileInfo)
    ...
    if strcmp(hFile.FileInfo(i).Type, 'nev')
        ...
        if isempty(nsFileInfo.FileComment)&&...
            ...
            nsFileInfo.FileComment =...
                fread(hFile.FileInfo(i).FileID, 256, '*char')';
            % if ripple app then app name is in file comment
            rpplAppName = strfind(nsFileInfo.FileComment,...
                'Ripple-Trellis.exe');
            if ~isempty(rpplAppName)
                AppEnd = strfind(nsFileInfo.FileComment, ']');
                nsFileInfo.AppName = nsFileInfo.FileComment(1:AppEnd);
                nsFileInfo.FileComment =...
                    deblank(nsFileInfo.FileComment(AppEnd+1:end));
            end
            % seek to Time Origin
