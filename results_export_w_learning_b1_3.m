...
%     nresults = results;
    ...
    for a=1:length(val);
        if ~strcmp(val{a},'conds')
            ...
            b11a = find(results(1:60,2)==1 & results(1:60,3)==1);
            b11c = find(results(1:60,2)==1 & results(1:60,3)==2);
            b33a = find(results(121:end,2)==1 & results(121:end,3)==1);
            b33c = find(results(121:end,2)==1 & results(121:end,3)==2);
            ...
            blist = {'b1a','b1c','b2a','b2c','b11a','b11c','b33a','b33c'};
            dlist = {'b1ac','b2ac','b11ac','b33ac'};
...
%                 nd(ot) = [];
            ...
            ddata=[diff(data(1:2)) diff(data(3:4)) diff(data(5:6)) diff(data(7:8))];
...
%         keyboard
        res.(disease).ot(id,stim+1)=numel(ot);
...
res.(disease).apt(id,:,stim+1) = apt;
res.(disease).av(id,:,stim+1) = av;
res.(disease).art(id,:,stim+1) = art;
res.(disease).amv(id,:,stim+1) = amv;
