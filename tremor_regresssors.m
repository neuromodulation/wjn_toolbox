function [Dnew,tfreq,r_r_trials,r_t_trials,p_r_trials,p_t_trials]=tremor_regressors(file,emgchans)
...
tend=D.indtrial('Undefined');
ntrials = tend(1)-1;
nconds = D.nconditions-1;
selection = 1:ntrials;
chan = emgchans;
ind = selection;
...
cfg =[];
emg=ft_channelselection(D.chanlabels(D.indchantype('EMG')),chan);
cfg.channel = emg;
cfg.keeptrials = 'yes';
rdata= ft_timelockanalysis(cfg, data);
cfg =[];
cfg.hpfilter = 'yes';
cfg.hpfreq  = 60;
cfg.hpfiltord = 5;
...
cfg.rectify = 'yes';
rdata = ft_preprocessing(cfg,rdata);
relabel = match_str(data.label,emg);
chan = emgchans;
for a =1:numel(relabel);
    data.label{relabel(a)} =['unrectified_' data.label{relabel(a)}];
    chan = [chan data.label{relabel(a)}];
end
cfg=[];
data = ft_appenddata(cfg,data,rdata);  

for a = 1:nconds;
    cfg =[];
    cfg.channel = chan;
    cfg.trials = D.indtrial(D.condlist{a});
    cfg.keeptrials = 'yes';
    tdata= ft_timelockanalysis(cfg, data);
    Ntrials=size(tdata.trial,1);
    Ntime=numel(tdata.time);
    ...
    cfg.taper = 'dpss';
    cfg.method = 'mtmfft';
    cfg.foilim     = [1 40]; % Frequency range
    cfg.tapsmofrq = 4; % spectral multitaper smoothing
    
    pow = ft_freqanalysis(cfg, tdata);
end
