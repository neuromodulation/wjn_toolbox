function [files, sequence, root, details] =dbs_subjects_berlin_v4(initials, on)
...
elseif details.berlin
    ...
    if  strcmp(initials,'PLFP43') 
        ...
        % due to differences in the montage between blocks the montage
    ...
    if  strcmp(initials,'PLFP47') || strcmp(initials,'PLFP48') || strcmp(initials,'PLFP50') || strcmp(initials, 'PLFP51') 
        % details.chanset      = fullfile(dbsroot, 'SPM','berlin_channels_bostsciseg.mat');
        % montage              = load(fullfile(dbsroot, 'SPM','berlin_montage_bostsciseg.mat'));
        [montage, label]	 = berlin_make_montage_bostsci_seg_ringV2(dbsroot, initials, '.0100');
        details.chanset = label;
     ...
     if  strcmp(initials,'PLFP49')
        % details.chanset      = fullfile(dbsroot, 'SPM','berlin_channels_bostsciseg.mat');
        % montage              = load(fullfile(dbsroot, 'SPM','berlin_montage_bostsciseg.mat'));
         % [montage, label]	 =  this is calculated below in the switch loop
        % due to differences in the montage between blocks the montage
        % calculation is done in the SWITCH command, adopt CODE BELOW
        details.eventchannel = 'EEG144'; % use empty channel, EEG144 is headbox input 32 (128+16: input 16+16 = datafile 16) 
        load_label = 0;
      end
...
%            details.chanset = label;
    ...
    case 'PLFP46'
     ...
     case 'PLFP47' % original name: 'DYST07'
        %DONE stop change names for montage at top as well
        if on
        else
            files = {
    ...
    case 'PLFP48' % original name: 'DYST07'
        %DONE stop change names for montage at top as well
        if on
        else   
            files = {
...
%             files = {
%                 'PLFP49/plfp49.0100.con',...
%                 'PLFP49/plfp49.0200.con',...    
%                 'PLFP49/plfp49.0300.con'};
%             root = 'PLFP49/';
%             
%             sequence = {'Ra','MVRLa','Rb'};
%             details.berlin = 1;
%             details.markers(1).files = {'plfp49.before1-coregis.txt'};
%             details.markers(2).files = {'plfp49.before2-coregis.txt'};
%             details.markers(3).files = {'plfp49.before3-coregis.txt'};
%            files = {
%                 'PLFP49/plfp49.0100.con',...
% 
%                };
...
%                };
            ...
            root = 'PLFP49/';
            sequence = {'Rb'}; % {'MVRLa'};% {'Ra'};
            details.berlin = 1;
            % details.markers(1).files = {'plfp49.before1-coregis.txt'};
            % details.markers(1).files = {'plfp49.before2-coregis.txt'};
            details.markers(1).files = {'plfp49.before3-coregis.txt'};
            % [montage, label]	 = strmatch(dbsroot, initials, '.0100');
            % [montage, label]	 = berlin_make_montage_bostsci_seg_ringV2(dbsroot, initials, '.0100');
            % [montage, label]	 = berlin_make_montage_bostsci_seg_ringV2(dbsroot, initials, '.0200');
            [montage, label]	 = berlin_make_montage_bostsci_seg_ringV2(dbsroot, initials, '.0300');
            details.chanset = label;
            ...
            for f=1:length(files)
             ...
             % these filenames just copied and index increased
            details.orestfile = fullfile(outroot,root,'SPMrest','tsssPLFP49_on.mat');
            details.otaskfiles = {fullfile(outroot,root,'SPMtask','PLFP49_off_MVRLa'),...
            fullfile(outroot,root,'SPMtask','PLFP49_off_MVRLa.mat')};
            details.left_trigger = 'EEG159';
            details.right_trigger = 'EEG160';
        end   
     case 'PLFP50'
        if on
            ...
            files = {
                'PLFP50/plfp50.0100.con',...
                'PLFP50/plfp50.0200.con'};
            root = 'PLFP50/';
            
            sequence = {'R','R'};
            details.berlin = 1;
            details.markers(1).files = {'plfp50.before1-coregis.txt'};
            details.markers(2).files = {'plfp50.before2-coregis.txt','plfp50.after2-coregis.txt'};
            details.flatthresh = 25;   
            details.lfpthresh = 0.3;   
            for f=1:length(files)
                [pathstr, name, ext] = fileparts([dbsroot filesep files{f}]);
            ...
            % this filenames just copied and index increased
            details.orestfile = fullfile(outroot,root,'SPMrest','tsssPLFP50_on.mat');
            details.left_trigger = 'EEG159';
            details.right_trigger = 'EEG160';
        end   
    case 'PLFP51' 
        if on
            ...
            files = {
                'PLFP51/plfp51.0100.con',...
                'PLFP51/plfp51.0200.con',...    
                'PLFP51/plfp51.0300.con'};
            root = 'PLFP51/';
            
            sequence = {'R','MVR','MVL'};
            details.berlin = 1;
            details.markers(1).files = {'plfp51.before1-coregis.txt'};
            details.markers(2).files = {'plfp51.before2-coregis.txt'};
            details.markers(3).files = {'plfp51.before3-coregis.txt'};
            details.flatthresh = 25;   
            details.lfpthresh = 0.3;   
            for f=1:length(files)
                [pathstr, name, ext] = fileparts([dbsroot filesep files{f}]);
            ...
            % these filenames just copied and index increased
            details.orestfile = fullfile(outroot,root,'SPMrest','tsssPLFP51_on.mat');
            details.otaskfiles = {fullfile(outroot,root,'SPMtask','PLFP51_off_MVR1'),...
            fullfile(outroot,root,'SPMtask','PLFP51_off_MVL1.mat')};
            details.left_trigger = 'EEG159';
            details.right_trigger = 'EEG160';
        end   
end
