...
function [pval, t_orig, crit_t, est_alpha, seed_state]=permTest_p1(data1,data2,n_perm,tail,alpha_level,mu,reports,seed_state)

if nargin<1,
    error('You need to provide data.');
end

if exist('data2','var') && length(data1) == length(data2);
    data = data1 - data2;
else
    data = data1;
end
...
if nargin<6 && exist('data2','var') && length(data2) == 1;
    mu=data2;
else 
    mu = 0;
end
