function wjn_lead_mapper(roi,s,f)
% function wjn_lead_mapper(roi,s,f)
% - Lead-DBS Job created on 26-Mar-2017:
% --------------------------------------
if ischar(roi)
    roi = {roi};
elseif isnumeric(roi)
    for  a = 1:size(roi,1)
        nroi{a} = wjn_spherical_roi(['roi_' num2str(a) '.nii'],roi(a,:),2);
    end
    roi = nroi;
end
fdir = cd;
...
options=getoptslocal(roi,fdir,f,s);
...
function options=getoptslocal(roi,fdir,f,s)
options.leadprod = 0;
...
options.dolc = 0;
options.lcm.seeds = roi;
options.lcm.seeddef = 'manual';
options.lcm.odir = fdir;
options.lcm.omask = [];
options.lcm.struc.do = s;
options.lcm.struc.connectome = 'HCP';
options.lcm.struc.espace = 1;
options.lcm.func.do = f;
options.lcm.func.connectome = 'resting_state>Full Set (Yeo 2011)'; %resting_state>First 100 subjects';
options.lcm.cmd = 1;
options.uipatdirs = [];
...
options.prefs.lc.datadir = ea_getconnectomebase;
...
options.prefs.machine.normsettings.ants_preset = 'ea_antspreset_effective';
...
% options.lcm.omask = [];
% options.lcm.struc.do = s;
% options.lcm.struc.connectome = 'ppmi';
% options.lcm.struc.espace = 1;
% options.lcm.func.do = f;
