...
files = ffind('ffibers_*.mat');
for a  =1:length(files)
    wjn_fiber_parcellation_ncon(files{a})
end
...
for a=res.PD.images
   ...
   fm=load(['parc_ffibers_' num2str(a)]);
%    sp(n,1) = numel(find(~ismember(fp.premotor.id,fm.motor.id)));
   fsm(n,1)=fm.sensorimotor.n%+fm.sensory.n;%+fp.premotor.n;
...
for a = 1:length(vars)
    ...
    wjn_corr_plot(eval(vars{a}),du,[.5 .5 .5])
    % xlim([0 2500])
    % ylim([-50 200])
    xlabel('Number of connecting fibers to premotor region')
    ylabel('Percentage ON and OFF difference')
...
%% spatial correlation with strongest response
...
% mi = find(m.img(:));
o = wjn_read_nii(res.PD.mresfiles{i});
o = o.img(:);

for a = 1:length(prt)
    nii=wjn_read_nii(res.PD.mresfiles{a});
    [r(a),p(a)] = wjn_permcorr(double(o),double(nii.img(:)));
end

[r,p]=wjn_permcorr(r',prt)
