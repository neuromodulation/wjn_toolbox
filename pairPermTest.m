function  [cont,tStat,repdiff] = pairPermTest(ni, data1,data2)

clear repdiff*
%permutation Test between two vectorial distributions 
% data1 = 1x ns1  or ns1 x nsampl
% data2 = 1x ns2  or ns1 x nsampl
...
% 	disp(['data should be in format 1 x ns'])
	...
	%joint distribution
	b=cat(2,data1,data2);  %1x(ns1+ns2)
	% FOR THOMAS: here new sample distribution 
	ds = size(data1);  %1 x ns
	b = [repmat(0,1,ds(2)) repmat(1,1,ds(2))];
