function simulate_source_coherence(S)
% S.folder
% S.D
% S.condition
% O.refchans
...
if ~isfield(S, 'refchans')
    [selection, ok]= listdlg('ListString', chanlabels(S.D), 'SelectionMode', 'single' ,'Name', 'Select reference channel' , 'ListSize', [400 300]);
    if ~ok
        return;
    end
    S.refchans = chanlabels(S.D, selection);
    S.refchans = S.refchans{1}; 
end



if ~isfield(S, 'freqranges')
    S.freqranges = spm_input('Frequency range (Hz):', '+1', 'r', '', 2);
end


if ~iscell(S.refchans);
    S.refchans = cellstr(S.refchans);
end
O.freqranges = S.freqranges;
O.refchans = S.refchans;
for freqi = 1:size(O.freqranges,1);
for refi = 1:numel(O.refchans)

D = S.D;
refchan = O.refchans{refi};
%%
res = mkdir(fullfile(S.folder, [refchan '_' num2str(O.freqranges(freqi,1)) '_' num2str(O.freqranges(freqi,2)) 'Hz']));
cd(fullfile(S.folder, [refchan '_' num2str(O.freqranges(freqi,1)) '_' num2str(O.freqranges(freqi,2)) 'Hz']));
outdir = pwd;
%%
range = O.freqranges(freqi,:);

S.centerfreq = mean(range);
S.tapsmofrq  = 0.5*diff(range);
%%
ranges = {[5 45], [55 95]};
rangeres = [2.5, 7.5];
...
mask = fullfile(S.folder, 'cohimages', refchan, rangelabels{f}, 'sigmask.nii');

ind = D.pickconditions(condition);
data = D.ftraw(0);
...
S.conditions = condition;
S.appendchannels = [D.chanlabels(strmatch(refchan, D.chanlabels)), {'stim', 'rec', 'recfixed'}];
