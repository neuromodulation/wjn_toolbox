...
% load(fullfile(path,infoname),'name','closedmode','circlesize','circledistance','scalingfactor','fixationcrosssize','pretrialtime','posttrialtime','order');
...
data.dXtarget = dummy;
data.dXtarget(2:end,1) =diff(data.AO0);
data.dYtarget = dummy;
data.dYtarget(2:end,1) = diff(data.AO1);
...
% 
% slocs = sort([Xlocs,Ylocs]);
x = find(sumdiff>=1);
n=0;
clear sdi
for a = 6:length(x)-6;
    if max(data.AO0(x(a)-5:x(a)+5)) >= 0.3 && max(data.AO1(x(a)-5:x(a)+5)) >= 0.3 && min(data.AO0(x(a)-5:x(a)+5)) <= 0.08 && min(data.AO1(x(a)-5:x(a)+5)) <= 0.08 
       
        if n<= 1
             n=n+1;
            sdi(n) = x(a);
        elseif diff([sdi(n-1),x(a)]) >= 4000;
            n=n+1;
            sdi(n) = x(a);
        end
    end
...
istart = [];
xmin = 0;xmax = 2000;
while xmax <= max(t*1000)-6000;


    plot(data.AO0,'color','k');
    hold on;
    plot(data.AO1,'color','r');
    plot(sumdiff,'color','b')
    xlim([xmin xmax+10000])
...
%         plot(data.AO1,'color','r');
%         plot(sumdiff,'color','b')
%         xlim([xdata(1) xdata(1)+500])
%         xxdata = ginput;
        
        istart(length(istart)+1,1) = xdata(1);
    end
end
...
green_intertrial = sdi(find(data.Xtarget(sdi) == 1))';
red_intertrial = sdi(find(data.Xtarget(sdi) == 2))';
