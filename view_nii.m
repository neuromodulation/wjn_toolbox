...
%1 Open files from a tree view file browser (Menu: File -> File Browser)
%2 Support scroll wheel to change view
%3 Click hold and move to change view, instead of clicking multiple times 
%4 Now load and show example MRI T1data, if no parameters passing in
%  i.e   view_nii();
%5 Adjust windown/level using right button (current axis) or both buttons (all axes)
% -Yi Sui (ysui2@uic.edu)
...
%load example
   ...
   case {'init'}
      ...
      set(fig,'ResizeFcn',@(varargin) view_nii(fig,struct('usestretch',0)));
...
%    wheelfun = get(fig, 'WindowScrollWheelFcn');
%    wheelfun = [wheelfun '; view_nii(''scroll_wheel'');'];
wheelfun=@scroll_wheel;
set(fig, 'WindowScrollWheelFcn', wheelfun);
set(fig, 'userdata',0);

set(fig, 'WindowButtonDownFcn',@WBDFcn);
set(fig, 'WindowButtonUpFcn',@WBUFcn);
return;						% init
...
function WBDFcn(varargin)
    ...
    if  isfield(G,'BtnDwnCounter')
        G.BtnDwnCounter = G.BtnDwnCounter+1;
    else
        G.BtnDwnCounter = 1;
    end
    ...
    if ismember(get(fh,'SelectionType'),{'alt','extend'})
        ...
        if  G.BtnDwnCounter > 1  %two or more bottons down
            setappdata(fh,'G',G);
            return;
        end
    ...
    setappdata(gcf,'flag_btndown',1);% flag of buttondown for view change (see move_cursor() )
    move_cursor(gcf)
    end
...
function WBUFcn(varargin)
    ...
    if ~strcmp(get(gcf,'SelectionType'),'normal')
        if G.BtnDwnCounter > 0
            setappdata(fh,'G',G);
            return;
        end
        try delete(G.txtbox);end
        opt.setcbarminmax=get(gca,'Clim');
        view_nii(fh,opt)
        set(fh,'WindowButtonMotionFcn',G.oldWBMFcn);
    end
    setappdata(gcf,'flag_btndown',0);% clear flag of buttondown for view change (see move_cursor() )
    setappdata(fh,'G',G);
...
%   if isfield(nii_view, 'colorlevel') & ~isempty(nii_view.colorlevel)
   ...
   function move_cursor(fig)
       ...
       flag_btndown=getappdata(gcf,'flag_btndown');
       if ~isempty(flag_btndown)&& isnumeric(flag_btndown) && flag_btndown==1
           view_nii(whichaxes);
       end
       return;					% move_cursor
%----------------------------------------------------------------
...
if ~isempty(slider)
    val = get(nii_view.handles.(slider),'Value');
    step = evnt.VerticalScrollCount;
    step = sign(step).* round(abs(step).^1.4);
    set(nii_view.handles.(slider),'Value',val+step);
    view_nii(slider);
end
