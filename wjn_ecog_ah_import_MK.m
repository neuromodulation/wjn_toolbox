clear
filename = 'MK02_LT.mat';

load(filename,'signal','CueStimulusTimes','CommandStimulusTimes','LeftResponseTimes','RightResponseTimes','FeedbackStimulusTimes','Force','time1kHz','labels');
...
for a = 1:length(chans)
    ...
    plot(f,rpow(a,:),'color','k','linewidth',3)
    xlim([3 40])
    ylim([0 1.2])
    xlabel('Frequency [Hz]')
    ylabel('Power [%]')
...
dlb=wjn_eeg_burst_analysis(Dcont.fullfile,[10 20],[],[sSTN sStrip],[],'brown');
...
xlim([5 40])
figone(7,30)
myprint('Burst power comparison 2')
...
myprint('Burst power correlations 2')
...
for a = 1:Dr.nchannels
    ...
    TFaxes
    figone(7,20)
    myprint(['TF_' Dr.chanlabels{a} ' 2'])
%     ylim([2 200])<<r
end
%%


De=wjn_sl('edo*MK*.mat');
...
%%
smspac = squeeze(nanmean(Dp(2,:,:,1),1));
smlpac = squeeze(nanmean(Dp(2,:,:,2),1));

cmspac = squeeze(nanmean(Dp(4,:,:,1),1));
cmlpac = squeeze(nanmean(Dp(4,:,:,2),1));
