...
% Last Modified by GUIDE v2.5 02-Apr-2018 13:32:42
...
disp('started')
for nx = 1:mg
    keep nx ops mp mn mi files va mnc npc mg outsize visible targetfolder frame handles int
    nd=[];
    while isempty(nd)
        n=randi(mn,1);
...
%     close all
    prc = nprc;
    inc = uint8(squeeze(prctile(nd,prc,1)));
%     keyboard
    if int
    inc = imresize(inc,[10000 10000],'bicubic');
    end
    disp('out')
...
%     print(of,fullfile(targetfolder,[fname '.jpg']),'-djpeg','-r1200')
